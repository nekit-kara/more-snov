				...
				<form action="<?php echo $action_import; ?>" method="post" id="form_product_import" enctype="multipart/form-data">
					<div class="g-row">
						<div class="g-6 f-form">
							...
							<div class="f-row">
								<label><?php echo $entry_key_field; ?> <?php echo $entry_import_manufacturer; ?></label>
								<div class="f-input">
									<select class="g-3" name="csv_import[key_field_manufacturer]">
										<option value="0" selected="selected"><?php echo $text_none; ?></option>
										<?php foreach ($manufacturers as $manufacturer) { ?>
										<option value="<?php echo $manufacturer['manufacturer_id']; ?>" <?php if(isset($csv_import['key_field_manufacturer']) && $csv_import['key_field_manufacturer'] == $manufacturer['manufacturer_id'] ){echo ' selected="selected"';}?>><?php echo $manufacturer['name']; ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							...
							<div class="f-row">
								...
								<div class="f-input">
									...
									<div class="scrollbox">
										...
										<?php foreach ($stores as $store) { ?>
										<?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
										<label class="<?php echo $class; ?>"><input type="checkbox" name="csv_import[to_store][]" value="<?php echo $store['store_id']; ?>" <?php if(isset($csv_import['to_store']) && in_array($store['store_id'], $csv_import['to_store'])){echo ' checked="checked"';}?> /> <?php echo $store['name']; ?></label>
										<?php } ?>
									</div>
									<a onclick="$(this).parent().find(':checkbox').prop('checked', true);"><?php echo $text_select_all; ?></a> / <a onclick="$(this).parent().find(':checkbox').prop('checked', false);"><?php echo $text_unselect_all; ?></a>
									</div>
