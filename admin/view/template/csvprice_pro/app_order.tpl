			...
			<div class="g-row">
				...
				<div class="tab-content">
					...
					<div id="tab_import" class="tab-pane fade">
						<form action="<?php echo $action_import; ?>" method="post" id="form_order_import" enctype="multipart/form-data" class="ocl-ui-form-horizontal">
							<div class="g-row">
								<div class="g-12 f-form">
									<div class="f-row">
										...
										<div class="f-input">
											<select name="csv_import[file_encoding]" class="g-3">
												<?php if (isset($charsets) && !empty($charsets)) { ?>
												<?php foreach ($charsets as $key => $val) { ?>
												<?php if ( isset($csv_import['file_encoding']) && $csv_import['file_encoding'] == $key ) { ?>
												<option value="<?php echo $key;?>" selected="selected"><?php echo $val;?></option>
												<?php } else { ?>
												<option value="<?php echo $key;?>"><?php echo $val;?></option>
												<?php } ?>
												<?php } ?>
												<?php } else { ?>
												<option value="UTF-8" <?php if ( isset($csv_import['file_encoding']) && $csv_import['file_encoding'] == 'UTF-8' ) echo 'selected'; ?>>UTF-8</option>
												<option value="WINDOWS-1251" <?php if ( isset($csv_import['file_encoding']) && $csv_import['file_encoding'] == 'WINDOWS-1251' ) echo 'selected'; ?>>Windows-1251</option>
												<?php } ?>
											</select>
										</div>
									</div>
									...
									<div class="f-row">
										<label><?php echo $entry_order_id; ?></label>
										<div class="f-input">
											<input class="g-3" type="text" name="csv_import[field_id]" value="<?php if(isset($csv_import['field_id'])) { echo $csv_import['field_id'];} else {echo '_ID_';} ?>">
										</div>
									</div>
									<div class="f-row">
										<label><?php echo $entry_order_status_id; ?></label>
										<div class="f-input">
											<input class="g-3" type="text" name="csv_import[field_status_id]" value="<?php if(isset($csv_import['field_status_id'])) { echo $csv_import['field_status_id'];} else {echo '_STATUS_ID_';} ?>">
										</div>
									</div>
									<div class="f-row">
										<label><?php echo $entry_order_status; ?></label>
										<div class="f-input">
											<input class="g-3" type="text" name="csv_import[field_status]" value="<?php if(isset($csv_import['field_status'])) { echo $csv_import['field_status'];} else {echo '_STATUS_';} ?>">
										</div>
									</div>
									<div class="f-row">
										<label><?php echo $entry_order_comment; ?></label>
										<div class="f-input">
											<input class="g-3" type="text" name="csv_import[field_comment]" value="<?php if(isset($csv_import['field_comment'])) { echo $csv_import['field_comment'];} else {echo '_COMMENT_';} ?>">
										</div>
									</div>
									<div class="f-row">
										<label data-prop_id="0"><?php echo $entry_order_notify; ?></label>
										<div class="f-input">
											<input class="g-3" type="text" name="csv_import[field_notify]" value="<?php if(isset($csv_import['field_notify'])) { echo $csv_import['field_notify'];} else {echo '_NOTIFY_';} ?>">
										</div>
									</div>
									<div class="f-row">
										<label><?php echo $entry_api; ?></label>
										<div class="f-input">
											<select name="csv_import[api_id]" id="input-api" class="g-3">
						                      <option value="0"><?php echo $text_none; ?></option>
						                      <?php foreach ($apis as $api) { ?>
						                      <?php if (isset($csv_import['api_id']) && $api['api_id'] == $csv_import['api_id']) { ?>
						...
						<div class="g-row"></div>
						<div class="g-row">
							<div class="f-row f-actions-left"><a onclick="$('#form_order_import').submit();" class="f-bu f-bu-default g-2"><?php echo $button_import;?></a></div>
						</div>
					</div>
					<!-- end tab_import -->
				</div>
