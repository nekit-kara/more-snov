...
<div class="<%= (parseInt(model.config.display) == 1) && model.show_shipping_address ? '' : 'hidden' %>">
	...
	<% if(model.account == 'logged'){ %> 
			...
			<% _.each (model.addresses, function(address) { %>
				<div class="list-group-item <%= address.address_id == model.shipping_address.address_id ? 'active' : '' %>">
		            <label for="shipping_address_exists_<%= address.address_id %>">  
		            	<input type="radio" name="shipping_address[address_id]" class="form-check-input form-check-input_custom shipping-address" value="<%= address.address_id %>" id="shipping_address_exists_<%= address.address_id %>" <%= address.address_id == model.shipping_address.address_id ? 'checked="checked"' : '' %> data-refresh="2" autocomplete='off' /> 
			...
			<% _.each (model.addresses, function(address) { %>
	          <div class="radio-input">
	          	<label for="shipping_address_address_id_<%= address.address_id %>">
	            	<input type="radio" name="shipping_address[address_id]" class="form-check-input form-check-input_custom shipping-address" value="<%= address.address_id %>" id="shipping_address_address_id_<%= address.address_id %>" <%= address.address_id == model.shipping_address.address_id ? 'checked="checked"' : '' %> data-refresh="2" autocomplete='off' />
		...
		<% } %>
		<div class="radio-input">
            <input type="radio" name="shipping_address[address_id]" class="form-check-input form-check-input_custom shipping-address" value="new" id="shipping_address_address_id_new" <%= model.shipping_address.address_id == 'new' ? 'checked="checked"' : '' %> data-refresh="2" autocomplete='off' />
