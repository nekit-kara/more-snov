	      		...
	      		<div id="<%= model.config.id %>_<%= f.id %>_input" 
			        ...
			        <div class="col-xs-12"> 
			          <textarea
			            name="<%= model.config.id %>.<%= f.id %>" 
			            id="<%= model.config.id %>_<%= f.id %>"
			            class="form-control_custom form-control validate <%= require ? 'required' : 'not-required' %> <%= f.type %> <%= f.id %>" 
			            autocomplete="<%= autocomplete %>" 
			            <% if(Number(config.design.placeholder)) {  %>placeholder="<%= require ? '*' : '' %> <%= htmlDecode(f.title).replace(':', '') %>"<% } %> 
			      ...
			      <div id="<%= model.config.id %>_<%= f.id %>_input" 
			        ...
			        <div class="col-xs-6"> 
			          <input type="text"
			            ...
			            id="<%= model.config.id %>_<%= f.id %>" 
			            value="<%= model[model.config.id][f.id] %>" 
			            class="form-control_custom form-control <%= f.mask ? 'qc-mask': '' %> <%= require ? 'required' : 'not-required' %> <%= f.id %>"
					    placeholder="* Телефон"
			            autocomplete="<%= autocomplete %>" 
			            <% if(f.mask){ %>
			            qc-mask="<%= f.mask %>"
